...
const FormConversational = () => {
  ...
  if (submitted || window.location.pathname === '/result') {
    return <ResultSummary formData={formData} />;
  }

  return (
    <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#f8fafc' }}>
      <Box sx={{ flexGrow: 1, py: { xs: 4, md: 8 }, px: { xs: 2, md: 0 }, display: 'flex', width: '100%' }}>
        <Box sx={{ display: 'flex', width: '100%', maxWidth: '1000px', mx: 'auto', gap: 4, flexDirection: { xs: 'column', md: 'row' }, alignItems: 'flex-start' }}>
          
          {/* Left Sidebar - Fixed Width */}
          <Box sx={{ display: { xs: 'none', md: 'block' }, width: '240px', flexShrink: 0 }}>
                ...
                {sectionLabels.map((label, idx) => (
                  <Box key={idx} sx={{ display: 'flex', alignItems: 'center', position: 'relative' }}>
                    <Box sx={{ 
                      ...
                      backgroundColor: activeSection === idx ? '#00529B' : (activeSection > idx ? '#cbd5e1' : '#f1f5f9'),
                    ...
                    {activeSection === idx && (
                      <Box sx={{ position: 'absolute', right: -20, top: '50%', transform: 'translateY(-50%)', width: 2, height: 32, backgroundColor: '#F4B301' }} />
                    )}
                  </Box>
                ))}
              </Box>
          ...
          {/* Main Card Area - Flex Grow */}
          <Box sx={{ flexGrow: 1, minWidth: 0, width: '100%' }}>
            
            {/* Mobile Progress Indicator (Hidden on Desktop) */}
            <Box sx={{ display: { xs: 'block', md: 'none' }, mb: 3 }}>
              <Box display="flex" justifyContent="space-between" alignItems="center" mb={1}>
                <Typography variant="caption" sx={{ fontWeight: 800, color: '#00529B', letterSpacing: '0.5px' }}>
                  STEP {activeQ + 1} OF {logicalToActual.length}
                </Typography>
                <Typography variant="caption" sx={{ fontWeight: 700, color: '#64748b' }}>
                  {sectionLabels[activeSection]}
                </Typography>
              </Box>
              <Box sx={{ width: '100%', height: '6px', backgroundColor: '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}>
                <Box 
                  sx={{ 
                    width: `${((activeQ + 1) / logicalToActual.length) * 100}%`, 
                    height: '100%', 
            ...
            >
              <Box sx={{ mb: 4, display: { xs: 'none', md: 'block' } }}>
                <Typography variant="overline" sx={{ color: '#00529B', fontWeight: 800, letterSpacing: '1px' }}>
                  PART {activeSection + 1} OF 3
                </Typography>
              ...
              <Box sx={{ mb: 3 }}>
                {logicalToActual.slice(0, activeQ).map((stepIdx) => {
                  ...
                  return (
                    ...
                    >
                      <Typography variant="body2" sx={{ color: '#64748b', fontWeight: 600, flex: 1, pr: 2 }}>
                        {label || q.title}
                      </Typography>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flex: 2, justifyContent: 'flex-end' }}>
                        <Typography variant="body2" sx={{ fontWeight: 800, color: '#0f172a', textAlign: 'right', wordBreak: 'break-word' }}>
                          {value || q.summary()}
                        </Typography>
                        <Button 
                          ...
                          sx={{ opacity: 0, transition: 'opacity 0.2s', color: '#00529B', minWidth: 'auto', p: 0.5 }}
                        >
